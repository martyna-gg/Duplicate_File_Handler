...
> ''')
    ...
    # create a dict with files of the same sizes with sizes as keys
    ...
    if decision3 == 'yes':
        files_hash = {key: {} for key in files_same_size}  # create a dict with files of the same sizes with hashes
        ...
        files_same_hash = {key: {} for key in files_hash}  # create a dict with files of same size and same hash
        ...
        if decision4 == 'yes':
            ...
            try:
                while not (decision5 != '' and all([int(x) in range(1, counter+1) for x in decision5.split(' ')])):
                    print("Wrong format")
                    decision5 = input('Enter file numbers to delete: >')
            except:
                print("Wrong format")
                decision5 = input('Enter file numbers to delete: >')
